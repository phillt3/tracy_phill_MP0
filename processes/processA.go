...
func main() {
	...
	-info includes (in order): recipient (To), Title of message, sender (From), date (Date), message (Content)
	*/
	arguments := os.Args //For collecting command-line run input (IP and desired port number)
	if len(arguments) == 1 {
	...
	if err != nil {
		//if error is returned by net.Dial,it is printed
		fmt.Println(err)
		return
	}
